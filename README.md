...
3. **Executes** tools in parallel (30+ threat intel providers)
...
| `src/lib/agent/tools.ts` | Tool registry — 30+ threat intel tools wrapping platform API endpoints |
...
| Category | Tools |
|----------|-------|
| **IOC & Reputation** | `check_ioc`, `correlate_iocs`, `get_ioc_lifecycle`, `get_trending_iocs`, `get_relationships` |
| **CVE & Vulns** | `lookup_cve` |
| **Domain & Network** | `lookup_domain`, `lookup_ip_geo`, `lookup_asn`, `get_domain_history`, `pivot_domain` |
| **Actors & Malware** | `enrich_actor`, `search_malpedia`, `search_triage` |
| **Intel Feeds** | `get_live_iocs`, `get_ransomware_activity`, `get_threat_pulse`, `get_today_briefing`, `get_detections`, `unified_search` |
| **Breach** | `check_breach` |
| **MITRE** | `lookup_mitre` |
| **Web & Phishing** | `scan_website`, `analyze_phishing_url`, `wayback_lookup` |
| **Crypto** | `trace_crypto_address` |
| **Reports** | `parse_threat_report` |
| **CT Monitoring** | `get_domain_certs` |
...
This agent is part of the [DFIR & ThreatIntel Portfolio Platform](https://pranithjain.qzz.io) — a comprehensive cybersecurity analysis platform with 230+ tools across DFIR and Threat Intelligence domains.
